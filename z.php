...
{
    include_once 'configMain.php';
    if(is_numeric($_GET['idLesson']) && is_numeric($_GET['idStud']) && is_numeric($_GET['idLessons']) && is_numeric($_GET['oldGrades']) && is_numeric($_GET['idF'])){
        if(isset($_GET['nGroup']) && $_GET['nGroup']){ $nGr=trim(substr($_GET['nGroup'],0,6)); } else { $nGr=""; }
        mysqli_query($dbMain, "UPDATE rating SET del=1 WHERE del=0 AND (pStatus=0 OR pStatus=2) AND idLesson=".$_GET['idLesson']." AND idStud=".$_GET['idStud']);

            $ar=str_split($_GET['oldGrades'], 2); $arcount = count($ar);
            for($iS=0; $iS<=($arcount-1); $iS++){ $strI['n'.$ar[$iS]]++; }
            $firstLine = 0;
            while(list($key,$val) = each($strI)){ if(!$firstLine){ $strUpdate=$key."=".$key."-".$val; $firstLine=1; } else { $strUpdate.=",".$key."=".$key."-".$val; } } 
            mysqli_query($dbMain, "UPDATE aerostat SET ".$strUpdate." WHERE idStud=".$_GET['idStud']." AND idLessons=".$_GET['idLessons']." AND idFak=".$_GET['idF']." AND nGroup='".$nGr."'");
    } else {
        echo "No";
    }
...
{
    include_once 'configMain.php';
    if(is_numeric($_GET['id_Zapis']) && is_numeric($_GET['idStudent']) && is_numeric($_GET['grades']) && is_numeric($_GET['oldGrades']) && is_numeric($_SESSION['SesVar']['FIO'][0])){
        if(isset($_GET['nGroup']) && $_GET['nGroup']){ $nGr=trim(substr($_GET['nGroup'],0,6)); } else { $nGr=""; }
        ...
        if (mysqli_num_rows($resTime) >= 1) {
            ...
            $Nn = 0;
            $ar=str_split($_GET['oldGrades'], 2); $arcount = count($ar);
            for($iS=0; $iS<=($arcount-1); $iS++){ if($ar[$iS]!=20 && $ar[$iS]!=21 && $ar[$iS]!=22){ $strI['n'.$ar[$iS]]++; } else { $Nn = 1; } }
            while(list($key,$val) = each($strI)){ $strUpdate.=",".$key."=".$key."-".$val; } 
            mysqli_query($dbMain, "INSERT INTO aerostat (idStud,idLessons,idFak,nGroup) VALUES (".$_GET['idStudent'].",".$_GET['idLessons'].",".$_GET['idF'].",'".$nGr."') ON DUPLICATE KEY UPDATE idStud=".$_GET['idStudent'].",idLessons=".$_GET['idLessons'].",idFak=".$_GET['idF'].",nGroup='".$nGr."'".$strUpdate);

            $ar=str_split($_GET['grades'], 2); $arcount = count($ar);
            for($iS=0; $iS<=($arcount-1); $iS++){ if($ar[$iS]!=20 && $ar[$iS]!=21 && $ar[$iS]!=22){ $strU['n'.$ar[$iS]]++; } else { $Nn = 0; } }
            $firstLine = 0;
            while(list($key,$val) = each($strU)){ if(!$firstLine){ $strUpdate=$key."=".$key."+".$val; $firstLine=1; } else { $strUpdate.=",".$key."=".$key."+".$val; } } 
            mysqli_query($dbMain, "UPDATE aerostat SET ".$strUpdate." WHERE idStud=".$_GET['idStudent']." AND idLessons=".$_GET['idLessons']." AND idFak=".$_GET['idF']." AND nGroup='".$nGr."'");


            if($arr[6]!=$_SESSION['SesVar']['FIO'][0] || $arr[0] > 10 || $arr[7]!=4){
                ...
                mysqli_query($dbMain, "UPDATE rating SET DateO=CURDATE(), TimeO=CURTIME(), RatingO=".$_GET['grades'].", idEmployess=".$_SESSION['SesVar']['FIO'][0].", levEmployess=4, Nn=".$Nn." WHERE del=0 AND (pStatus=0 OR pStatus=2) AND id=" . $_GET['id_Zapis'] . " AND idStud=" . $_GET['idStudent']);
            }else{
                mysqli_query($dbMain, "UPDATE rating SET RatingO=".$_GET['grades'].", idEmployess=".$_SESSION['SesVar']['FIO'][0].", levEmployess=4, Nn=".$Nn." WHERE del=0 AND (pStatus=0 OR pStatus=2) AND id=".$_GET['id_Zapis']." AND idStud=".$_GET['idStudent']);
            }
...
function addLessonStudent()
{
    ...
    if(is_numeric($_GET['idLessons']) && is_numeric($_GET['idStudent']) && is_numeric($_GET['PL']) && is_numeric($_GET['PKE']) && is_numeric($_GET['grades']) && is_numeric($_SESSION['SesVar']['FIO'][0]) && is_numeric($_GET['idLess'])){
        ...
        if (!mysqli_num_rows($res)) {
            $ar=str_split($_GET['grades'], 2); $arcount = count($ar);
            for($iS=0; $iS<=($arcount-1); $iS++){ $strI['n'.$ar[$iS]]++; }
            while(list($key,$val) = each($strI)){ $strInsert.=",".$key; $strInsertValue.=",".$val; $strUpdate.=",".$key."=".$key."+".$val; } 

           mysqli_query($dbMain, "INSERT INTO aerostat (idStud,idLessons,idFak,nGroup".$strInsert.") VALUES (".$_GET['idStudent'].",".$_GET['idLessons'].",".$_GET['idF'].",'".$nGr."'".$strInsertValue.") ON DUPLICATE KEY UPDATE idStud=".$_GET['idStudent'].",idLessons=".$_GET['idLessons'].",idFak=".$_GET['idF'].",nGroup='".$nGr."'".$strUpdate);
           mysqli_query($dbMain, "INSERT INTO rating (idLessons,idStud,PL,PKE,DateO,TimeO,RatingO,idEmployess,idLesson,nGroup,levEmployess) VALUES (".$_GET['idLessons'].",".$_GET['idStudent'].",".$_GET['PL'].",".$_GET['PKE'].",'".$dt[2]."-".$dt[1]."-".$dt[0]."',CURTIME(),".$_GET['grades'].",".$_SESSION['SesVar']['FIO'][0].",".$_GET['idLess'].",'".$nGr."',4)");
...
function GroupViewL()
{
    ...
    $retVal .= "Группа № ".$_GET['nGroup']." (<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF'].(isset($_GET['idFI']) && $_GET['idFI'] ? "&idFI=".$_GET['idFI'] : "")."&idGroup=".$_GET['idGroup']."&PL=0&nPredmet=".$_GET['nPredmet']."&nF=".$_GET['nF']."&nGroup=".$_GET['nGroup']."&IdCaptain=".$_GET['IdCaptain']."'>Практическое</a> / ЛЕКЦИЯ)</h3><hr>";
    ...
    <input type='hidden' id='idF' value='".(isset($_GET['idFI']) && $_GET['idFI'] ? $_GET['idFI'] : $_GET['idF'])."'>
            ...
            if (mysqli_num_rows($resultL) >= 1) {
                while ($arr = mysqli_fetch_row($resultL)) {
                    ...
                    for ($iS = 0; $iS <= ($countarrStud - 1); $iS++) {
                        ...
                        if (!$trueS) {
                            $prepreRating .= "<div class='grade' data-idLes=".$arr[0]." data-idStudent=".$arrStud[$iS]." data-PKE=".$arr[2]." data-zapis=0 data-Status=0 data-Nn=0></div>\n";
                        }
                    }
...
function GroupViewP()
{
    ...
    $retVal .= "Группа № ".$_GET['nGroup']." (ПРАКТИЧЕСКОЕ / <a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF'].(isset($_GET['idFI']) && $_GET['idFI'] ? "&idFI=".$_GET['idFI'] : "")."&idGroup=".$_GET['idGroup']."&PL=1&nPredmet=" . $_GET['nPredmet'] . "&nF=" . $_GET['nF'] . "&nGroup=" . $_GET['nGroup'] . "&IdCaptain=".$_GET['IdCaptain']."'>Лекция</a>)</h3><hr>";
    ...
    if (mysqli_num_rows($resPredmet) >= 1) {
        ...
        if (mysqli_num_rows($rescourse) >= 1) {
            if ($_GET['idF'] == 283) {
                ...
                $sqlSO="((IdF=".$_GET['idF']." AND (".$sqlSr.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) OR ((".$sqlS.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4)) AND ((IdF=".$_GET['idF']." AND (".$sqlSr.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4) OR ((".$sqlSS.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4))";
            ...
            if ($_GET['idF'] == 283) {
                $sqlSO="((IdF=".$_GET['idF']." AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) OR (LEFT(Name,1)='".$fac[$_GET['idF']][0]."' AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4)) AND ((IdF=".$_GET['idF']." AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4) OR (LEFT(Name,1)='".$fac[$_GET['idF']][2]."' AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4))";
            } else {
      ...
      ";
            $result = mssql_query("SELECT IdGroup, Name, IIF(IdF != ".$_GET['idF'].", IdF, 0), IIF(IdCaptain!=null, IdCaptain, 0), IdF FROM dbo.Groups WHERE ".$sqlSO." ORDER BY Name, IdF", $dbStud);
            if (mssql_num_rows($result) >= 1) {
                $preChar = "";
                while ($arr = mssql_fetch_row($result)) {
                    ...
                    $retVal .= "<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']['Zav'][2]."&idPredmet=" . $_GET['idPredmet'] . "&idF=".$arr[4].($arr[2] ? "&idFI=".$arr[2] : "")."&idGroup=" . $arr[0] . "&PL=0&nPredmet=" . $Pre . "&nF=" . $idName . "&nGroup=" . $arr[1] . "&IdCaptain=".$arr[3]."'>Практ.</a>&nbsp;&nbsp;&nbsp;";
                    $retVal .= "<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']['Zav'][2]."&idPredmet=" . $_GET['idPredmet'] . "&idF=".$arr[4].($arr[2] ? "&idFI=".$arr[2] : "")."&idGroup=" . $arr[0] . "&PL=1&nPredmet=" . $Pre . "&nF=" . $idName . "&nGroup=" . $arr[1] . "&IdCaptain=".$arr[3]."'>Лекция</a>";
                    $retVal .= "</div></div></div>\n";
                }
            }
            $retVal .= "</div>";
        ...
        <fieldset>
            <div class='box'>
                ...
                <div class='number_theme'>
                    ...
                    <input type='text' id='number_theme' maxlength='3' onkeydown='return checkNumberThemeLesson(event);' onkeyup=\"this.value=this.value.replace(/[^0-9]/,'');\">
                </div>
...
<div id=\"form-edit-date\" title=\"Редактирование даты занятия\">
    <form>
        <fieldset>
            ...
            <div class='number_theme'>
                <b align='center'>№ темы</b><br>
                <input type='text' id='edit_number_theme' maxlength='3' onkeydown='return checkNumberThemeLesson(event);' onkeyup=\"this.value=this.value.replace(/[^0-9]/,'');\">
            </div>
            <br>
