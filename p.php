...
//        if(isset($_GET['nGroup']) && $_GET['nGroup']){ $nGr=trim(substr($_GET['nGroup'],0,6)); } else { $nGr=""; }
        $resTime = mysqli_query($dbMain, "SELECT TIMESTAMPDIFF(MINUTE, CONCAT(DateO, ' ', TimeO), NOW()), idLessons, idLesson, DateO, TimeO, RatingO, idEmployess, levEmployess, nGroup FROM rating WHERE del=0 AND id=".$_GET['id_Zapis']." AND idStud=".$_GET['idStudent']);
        if (mysqli_num_rows($resTime) >= 1) {
            ...
            mysqli_query($dbMain, "INSERT INTO aerostat (idStud,idLessons,idFak,nGroup) VALUES (".$_GET['idStudent'].",".$_GET['idLessons'].",".$_GET['idF'].",'".$arr[8]."') ON DUPLICATE KEY UPDATE idStud=".$_GET['idStudent'].",idLessons=".$_GET['idLessons'].",idFak=".$_GET['idF'].",nGroup='".$arr[8]."'".$strUpdate);
            ...
            mysqli_query($dbMain, "UPDATE aerostat SET ".$strUpdate." WHERE idStud=".$_GET['idStudent']." AND idLessons=".$_GET['idLessons']." AND idFak=".$_GET['idF']." AND nGroup='".$arr[8]."'");

            if($arr[6]!=$_SESSION['SesVar']['FIO'][0] || $arr[0] > 10 || $arr[7]!=3){
                ...
                mysqli_query($dbMain, "UPDATE rating SET DateO=CURDATE(), TimeO=CURTIME(), RatingO=".$_GET['grades'].", idEmployess=".$_SESSION['SesVar']['FIO'][0].", levEmployess=3, Nn=".$Nn." WHERE del=0 AND (pStatus=0 OR pStatus=2) AND id=" . $_GET['id_Zapis'] . " AND idStud=" . $_GET['idStudent']);
            }else{
                mysqli_query($dbMain, "UPDATE rating SET RatingO=".$_GET['grades'].", idEmployess=".$_SESSION['SesVar']['FIO'][0].", levEmployess=3, Nn=".$Nn." WHERE del=0 AND (pStatus=0 OR pStatus=2) AND id=".$_GET['id_Zapis']." AND idStud=".$_GET['idStudent']);
            }
...
function addLessonStudent()
{
    ...
    if(is_numeric($_GET['idLessons']) && is_numeric($_GET['idStudent']) && is_numeric($_GET['PL']) && is_numeric($_GET['PKE']) && is_numeric($_GET['grades']) && is_numeric($_SESSION['SesVar']['FIO'][0]) && is_numeric($_GET['idLess'])){
        ...
        if (!mysqli_num_rows($res)) {
            $ar=str_split($_GET['grades'], 2); $arcount = count($ar);
//            for($iS=0; $iS<=($arcount-1); $iS++){ $strInsert.=",n".$ar[$iS]; $strInsertValue.=",1"; $strUpdate.=", n".$ar[$iS]."=n".$ar[$iS]."+1"; }

            for($iS=0; $iS<=($arcount-1); $iS++){ $strI['n'.$ar[$iS]]++; }
            while(list($key,$val) = each($strI)){ $strInsert.=",".$key; $strInsertValue.=",".$val; $strUpdate.=",".$key."=".$key."+".$val; } 
/*
            for($iS=0; $iS<=($arcount-1); $iS++){ $strI['n".$ar[$iS]."']++; $strU['n".$ar[$iS]."']++; }
            while(list($key,$val) = each($strI)){ $strInsert.=",".$key; $strInsertValue.=",".$strI[$key]; } 
            while(list($key,$val) = each($strU)){ $strUpdate.=",".$key."=n".$key."+".$strU[$key]; } 
*/
           ...
           mysqli_query($dbMain, "INSERT INTO rating (idLessons,idStud,PL,PKE,DateO,TimeO,RatingO,idEmployess,idLesson,nGroup,levEmployess) VALUES (".$_GET['idLessons'].",".$_GET['idStudent'].",".$_GET['PL'].",".$_GET['PKE'].",'".$dt[2]."-".$dt[1]."-".$dt[0]."',CURTIME(),".$_GET['grades'].",".$_SESSION['SesVar']['FIO'][0].",".$_GET['idLess'].",'".$nGr."',3)");
...
function GroupViewL()
{
    ...
    <input type='hidden' id='idF' value='".$_GET['idF']."'>
...
//                $sqlSO="((IdF=".$_GET['idF']." AND (".$sqlSr.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) OR ((".$sqlS.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<=".$fac[$_GET['idF']][1]." AND LEN(Name)>=4)) OR ((IdF=".$_GET['idF']." AND (".$sqlSr.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4) OR ((".$sqlSS.") AND DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<=".$fac[$_GET['idF']][3]." AND LEN(Name)>=4))";
      ...
      ";
            $result = mssql_query("SELECT IdGroup, Name, IIF(IdF != ".$_GET['idF'].", 1, 0), IIF(IdCaptain!=null, IdCaptain, 0), IdF FROM dbo.Groups WHERE ".$sqlSO." ORDER BY Name, IdF", $dbStud);
            if (mssql_num_rows($result) >= 1) {
                $preChar = "";
                while ($arr = mssql_fetch_row($result)) {
                    ...
                    $retVal .= "<a href='p.php?menuactiv=goG&idPrepod=" . $_SESSION['SesVar']['FIO'][0] . "&idKaf=" . $_SESSION['SesVar']['Prepod'][2] . "&idPredmet=" . $_GET['idPredmet'] . "&idF=" . $arr[4] . "&idGroup=" . $arr[0] . "&PL=0&nPredmet=" . $Pre . "&nF=" . $idName . "&nGroup=" . $arr[1] . "&IdCaptain=".$arr[3]."'>Практ.</a>&nbsp;&nbsp;&nbsp;";
                    $retVal .= "<a href='p.php?menuactiv=goG&idPrepod=" . $_SESSION['SesVar']['FIO'][0] . "&idKaf=" . $_SESSION['SesVar']['Prepod'][2] . "&idPredmet=" . $_GET['idPredmet'] . "&idF=" . $arr[4] . "&idGroup=" . $arr[0] . "&PL=1&nPredmet=" . $Pre . "&nF=" . $idName . "&nGroup=" . $arr[1] . "&IdCaptain=".$arr[3]."'>Лекция</a>";
                    $retVal .= "</div></div></div>\n";
                }
            }
...
function HeaderFooterGroup($content, $title, $vC = '', $vS = '')
{
    ...
    <?php echo $content; ?>
    <div style="clear:both;">&nbsp;</div>
    <div class="export" alt="Экспортировать в Excel">
        <a href="#" title="Экспортировать в Excel"><img src="img/csv.png"></a>
    </div>
    <div id="comment" title="Содержание комментария будет видено только вам">Оставить комментарий
    	<!-- <div id="contentComment">
    		<textarea rows="10" cols="45" maxlength="2500" placeholder="Напишите здесь..."></textarea><br>
...
<div id='form-comment'>
    <form>
	   	<textarea rows='10' cols='45' placeholder='Напишите здесь...'></textarea><br>
    	<button id='saveComment'>Сохранить</button>
    </form>
</div>
